...
async def import_recipe_from_url(
    ...
    user = ctx.context.user
    return await _run_sync(_start_import_from_url, user, url)


def _start_import_from_url(user, url: str) -> ImportJobStarted:
    ...
    import_request = RecipeImportRequest.objects.create(
        user=user,
        payload={'url': url, 'source_type': 'imported'},
    ...
    return ImportJobStarted(request_id=str(import_request.id), url=url)
